...
Written by Trevor Gratz (trevormgratz@gmail.com)

This file maps nationwide changes in housing values next to demographic
characteristics. Currently, it takes in 3 sources of data census: data, zillow
data, and Tiger/Lines shape file. These files are merged at the zip code
tabulation area(zcta). The user is allowed to pick the census varialbe
(currently only supports race/ethnicity), the years of housing data to compare,
and the geographic area to investigate. The file produces one plot of two
subplots.
"""
...
ziptot = 0
while ziptot < 5:
    ziplist = input('Enter a city (e.g., Seattle) or list of zip codes' +
                    'separated by commas (e.g., 98101, 98102,...,98191): ')
    ...
    if any(i.isdigit() for i in ziplist):
        ...
        if any([len(i) != 5 for i in ziplist]):
            ziptot = 0
            print('\nAt least one entry had more or less than 5 digits.' +
                  'Use leading zeros if need be. Please try again.')
            continue

    else:
        maptitle = ziplist
        state = input('Please enter the 2 letter state' +
                      'abbreviation (e.g., WA): ')
    ...
    if ziptot < 5:
        print('\nFewer than 5 zip codes were selected. Please check the city' +
              'name if entered, or enter more zip codes')
...
if c == '':
    ck = input('Enter your census key, or set up a private key file and' +
               'start over: ')
    c = Census(ck)

demodict = {'Total Housing Units': 'P001001', 'Total Race': 'P007001',
            'White': 'P007003', 'Black': 'P007004',
            'American Indian or Alaskan Native': 'P007005',
            'Asian': 'P007006',
            'Native Hawaiian and Other Pacific Islander': 'P007007',
            'Other Race': 'P007008', 'Two or More Races': 'P007009',
            'Latino': 'P007010'}
...
while ((syear not in list(range(1996, 2020))) or
       (eyear not in list(range(1996, 2020)))):

    syear = int(input('Enter the beginning year to track home prices' +
                      '(1996-2019): '))
    eyear = int(input('Enter the last year to track home prices(1996-2019): '))

    if ((syear not in list(range(1996, 2020))) or
        (eyear not in list(range(1996, 2020)))):
...
# Get Census data
cendf, miscenzip = cp.get_censusdata(key=c, ziplist=ziplist,
                                     demo=demodict[demo], year=2010)
...
scatterdf = pd.merge(zildf, cendf, how='inner', left_on=['zipcode'],
                     right_on=['zipcode'])

# Merge Shape File and Merge Price Data Zillow Data
pricesshp = pd.merge(mapshp, zildf, how='left', left_on='ZCTA5CE10',
                     right_on='zipcode')
...
if (~zildf['per_change'].isnull()).sum() >= 5:
    ...
    p1 = pricesshp.plot(column='per_change', cmap=price_cmap, vmin=vmin,
                        vmax=vmax, ax=axm)
    ...
    # Plot Color Bar
    sm = ScalarMappable(cmap=price_cmap,
                        norm=plt.Normalize(lowbound, uppbound))
    tickmarks = [i for i in range(lowbound, uppbound + 25, 25)]
    cbar = plt.colorbar(sm, ticks=tickmarks, ax=axm, fraction=0.04)
    cbar.ax.set_yticklabels(list(map(lambda i: ('+' if i > 0 else "") + str(i),
                                     tickmarks)))
    ...
    pr = np.corrcoef((ys/xs)[~(ys/xs).isnull()],
                     scatterdf.loc[~(ys/xs).isnull(), cenvar])[0, 1].round(2)
    ...
    sc = axt.scatter(xs, ys, c=scatterdf[cenvar], cmap=race_cmap,
                     label='Zip Codes', vmin=lb, vmax=ub)
